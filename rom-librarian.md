...
`user.json` is the deployment-specific overlay. It may contain machine paths, frontend choices, per-system status, overrides, and learned notes. The user file wins over `static.json` when both define the same field.
...
- Whether game folders are incorrectly named with file extensions, such as `<game>.m3u/`.
...
### EmulationStation `.m3u` Folder Repair

Applies to RetroBat, Batocera, ES-DE, and related ES frontends when games are stored as folders named `<game>.m3u/`.
...
- Preserve path separator style expected by the frontend, based on `static.json`.
...
- No game folders remain with accidental `.m3u` directory suffixes unless intentionally preserved.
...
- `es-m3u-folder-name`: folders named `<game>.m3u/` are not playlist files.
