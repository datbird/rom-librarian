...
if (planPath === "--help" || planPath === "-h" || !planPath) {
  console.log("Usage: node scripts/apply-orphaned-media-quarantine.mjs <repair-plan.json> --apply [--allow-real-targets --confirm-target <absolute-target>]");
...
for (const step of plan.steps || []) {
  ...
  verification.push({ media_path: finding.media_path, verified: true, check: "source_removed_and_quarantine_exists" });
}
...
emitJson({
  ...
  notes: ["Only orphaned media files were moved to .rom-librarian-quarantine. No ROM, metadata, BIOS, firmware, key, or save files were modified."]
});
